...
public class ScheduleService {
	...
	public void parseActivities(final String fileName) throws FileNotFoundException {
				...
				if(startTime.compareTo(currentActivityEndsAt) >= 0) {
					...
					if(!validActivity.isEmpty()) {						
						...
						if(isLunchTime && !(Boolean) teamsDetailsMap.get(IS_LUNCH_BREAK_DONE)) {
							...
							scheduleStringBuilderForTeamI.append(NEW_LINE).append(startTimeString).append(SPACE_COLUN_SPACE).append(validActivity).append(SPACE).append(activitiesMap.get("time").toString()).append("min");
